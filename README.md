...
O sistema gerencia duas entidades principais com relacionamento de 1:N:
1. **CATEGORIA:** Classifica os tipos de ativos de T.I. (Ex: Hardware, Software, Periféricos).
2. **ITEM:** Detalha os bens patrimoniais individuais, contendo valor estimado, responsável e localização física.

   # 🚀 Sistema de Inventário — Setor de T.I.

[cite_start]Este repositório contém o código-fonte do **Projeto Final**  [cite_start]desenvolvido para a disciplina de **Banco de Dados** [cite: 12] [cite_start]do curso de **Tecnólogo em IA (2º Período)** [cite: 13] [cite_start]do **PIT — Piauí Instituto de Tecnologia**, sob a orientação da **Profa. [cite_start]Evelyn Silva**.

[cite_start]O objetivo do sistema é gerenciar com eficiência os ativos tecnológicos de uma organização (hardwares, softwares, licenças e periféricos), implementando uma arquitetura web persistente e integrada[cite: 29].

---

## 🔗 Links de Acesso Direto
[cite_start]Conforme os requisitos de entrega da atividade, seguem os links públicos do ecossistema do projeto:
* [cite_start]**Aplicação em Produção (Render):** [INSIRA_AQUI_O_LINK_DO_SEU_RENDER] [cite: 24, 31]
...
* [cite_start]**Banco de Dados Relacional:** Hospedado na nuvem através do **NEON (PostgreSQL)**[cite: 23, 27], garantindo alta disponibilidade, transações ACID confiáveis e persistência íntegra de dados.
* [cite_start]**Back-end e API:** Construído em **Python** utilizando o framework **Flask**[cite: 21, 22], responsável pelo gerenciamento de rotas, consumo de dados e barramento lógico da aplicação via ORM (*Flask-SQLAlchemy*).
* **Front-end e Telas:** Desenvolvido de forma responsiva utilizando **HTML5**, **CSS3 embedded** para garantir a renderização visual do design de forma consistente e **JavaScript** nativo para validações lógicas e comportamentais em tempo de execução[cite: 19, 25].
...
1. [cite_start]Os dados estruturais e os dados de inserção iniciais foram injetados por meio do console de edição do **Neon Tech (PostgreSQL)**.
2. [cite_start]O código-fonte foi gerenciado e versionado diretamente pelo editor web do **GitHub**[cite: 26].
3. [cite_start]O deploy contínuo (*CI/CD*) foi configurado no painel de serviços do **Render**, que mapeia o arquivo `requirements.txt` para instalar as bibliotecas do servidor automaticamente em ambiente Linux de produção, injetando as credenciais do banco por variáveis de ambiente ocultas (`DATABASE_URL`), preservando a segurança da aplicação.
